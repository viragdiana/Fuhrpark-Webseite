...
foreach ($insurances as $ins) {
    ...
    if ($ablaufTs <= $warningLimit) $expiringSoonCount++;
    if ($kuendigungTs >= $today && $kuendigungTs <= $warningLimit) $cancellationSoonCount++;
}
?>
...
<main class="max-w-[1300px] mx-auto px-4 sm:px-6 lg:px-8 py-4">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <a href="../../index.php" class="w-full sm:w-auto flex items-center justify-center gap-2 bg-primary text-primary-foreground hover:opacity-90 px-4 py-2.5 text-sm font-medium rounded-md shadow-sm transition-opacity" title="Gehen Sie zum Fahrzeugprofil, um eine Police hinzuzufügen">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div class="bg-card rounded-xl p-6 shadow-sm border border-border flex items-center gap-5">
            ...
            <div>
                <div class="text-sm text-muted-foreground font-medium">Gesamt Versicherungen</div>
            ...
            <table class="w-full text-sm text-left whitespace-nowrap">
                ...
                <?php foreach ($insurances as $ins):
                    ...
                    if ($isExpired) {
                        ...
                        $statusColor = 'text-[#c75146]'; // using destructive color for warning
                        $statusIcon = 'bi-exclamation-circle';
                    }
                    ?>
                ...
                <?php if (empty($insurances)): ?>
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-muted-foreground">
                            <i class="bi bi-shield-x text-4xl mb-3 block text-border"></i>
                            Es wurden keine Versicherungen im System gefunden.
                        </td>
                    </tr>
                <?php endif; ?>
                </tbody>
